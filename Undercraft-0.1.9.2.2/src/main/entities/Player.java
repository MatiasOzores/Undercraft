...
public class Player {
    ...
    public Player(int x, int y, int width, int height, int speed, Color color, 
            ...
            Image playerImageLeft1, Image playerImageLeft2, Image playerImageLeftStatic, Image playerImageRightStatic, Image corazonCompleto, List<Plataforma> plataformas) {
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
    ...
    public void draw(Graphics g) {
        // Dibuja al jugador con la imagen si estï¿½ disponible
        ...
        if (corazonCompleto != null) {
            ...
            int inicioX = 50; // Posiciï¿½n X inicial para dibujar corazones
            int inicioY = 30; // Posiciï¿½n Y para dibujar corazones
    ...
    public boolean verificarSaltoPlataforma() {
        // Asume que el jugador está sobre la plataforma si el borde inferior del jugador está cerca del borde superior de la plataforma
        boolean enPlataforma = false;
        
        for (Plataforma plataforma : plataformas) {
            // Verifica si el jugador está en el rango horizontal de la plataforma
            if (x + width > plataforma.getX() && x < plataforma.getX() + plataforma.getWidth()) {
                // Verifica si el jugador está en el rango vertical de la plataforma
                if (y + height <= plataforma.getY() + 10 && y + height + velocidadY >= plataforma.getY()) {
                    enPlataforma = true;
                    y = plataforma.getY() - height; // Ajusta la posición del jugador para que esté justo encima de la plataforma
